...
# Helper for frontend metadata
async def get_subject_meta(subject_id, detail_path=""):
    meta = {
    ...
    if not subject_id: return meta
    
    if not detail_path: detail_path = "details"
    url = f"{API_BASE}/wefeed-h5api-bff/detail?detailPath={detail_path}&subjectId={subject_id}"
    try:
        data = await _make_request(url)
        inner = data.get("data", {}).get("subject", {})
        if inner:
            meta["title"] = f"{inner.get('title', 'Watch')} - Streamfit"
            meta["description"] = inner.get("description", meta["description"])
...
def serve_html(filename: str, meta_replacements=None):
    ...
    if meta_replacements:
        html = html.replace("<title>Explore Movies - Streamfit</title>", f"<title>{meta_replacements.get('title', '')}</title>")
        html = html.replace("<title>Explore TV Series - Streamfit</title>", f"<title>{meta_replacements.get('title', '')}</title>")
...
async def movies(): return serve_html("public/movies.html", {"title": "Explore Movies - Streamfit"})

@app.get("/tv", response_class=HTMLResponse)
async def tv(): return serve_html("public/tv.html", {"title": "Explore TV Series - Streamfit"})

@app.get("/live-tv", response_class=HTMLResponse)
async def livetv(): return serve_html("public/live-tv.html", {"title": "Live TV - Streamfit"})

@app.get("/download", response_class=HTMLResponse)
async def download(): return serve_html("public/download.html")

@app.get("/details", response_class=HTMLResponse)
async def details(id: str = None, tmdb: str = None, type: str = "movie"):
    meta = await get_subject_meta(id)
    html = serve_html("public/details.html").body.decode()
    html = html.replace('<title>Details - Streamfit</title>', f"<title>{meta['title']}</title>")
    html = html.replace('id="detailsTitle">Title', f'id="detailsTitle">{meta["title"]}')
    html = html.replace('id="watchDescription">Description loading...', f'id="watchDescription">{meta["description"]}')
    html = html.replace('src="/default-cover.png"', f'src="{meta["cover"]}"')
    return HTMLResponse(content=html)

@app.get("/watch", response_class=HTMLResponse)
async def watch(id: str = None, tmdb: str = None, type: str = "movie"):
    meta = await get_subject_meta(id)
    html = serve_html("public/watch.html").body.decode()
    html = html.replace('<title>Watch Online - Streamfit</title>', f"<title>{meta['title']}</title>")
    return HTMLResponse(content=html)
...
async def get_banners():
    ...
    for op in data.get("data", {}).get("operatingList", []):
        if op.get("type") == "BANNER":
            items.extend(op.get("banner", {}).get("items", []))
...
async def api_filter(tabId: int = 1, sort: str = "RECOMMEND", page: int = 1, perPage: int = 24):
    ...
    if "data" in data and "items" not in data["data"] and "subjects" in data["data"]:
        data["data"]["items"] = data["data"]["subjects"]
    return data
...
async def api_detail(detailPath: str = "", subjectId: str = ""):
    url = f"{API_BASE}/wefeed-h5api-bff/detail?detailPath={detailPath}&subjectId={subjectId}"
    data = await _make_request(url)
...
async def api_resource(subjectId: str, se: int = 1, ep: int = 1, detailPath: str = ""):
    ...
    player_referer = f"{domain}/spa/videoPlayPage/movies/{detailPath}?id={subjectId}&type=/movie/detail&detailSe={se}&detailEp={ep}&lang=en"
    play_url = f"{domain}/wefeed-h5api-bff/subject/play?subjectId={subjectId}&se={se}&ep={ep}&detailPath={detailPath}"
    print(f"DEBUG: domain='{domain}', play_url='{play_url}'")
...
async def api_captions(subjectId: str, se: int = 1, ep: int = 1, detailPath: str = ""):
    dom_data = await _make_request(f"{API_BASE}/wefeed-h5api-bff/media-player/get-domain")
    domain = dom_data.get("data", "https://netfilm.world").rstrip("/")
    if not detailPath: detailPath = "details"
    ...
    cap_url = f"{API_BASE}/wefeed-h5api-bff/subject/caption?format={stream_format}&id={stream_id}&subjectId={subjectId}&detailPath={detailPath}"
